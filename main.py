...
def check_login_status(port):
    """Flask uygulamasında kullanıcı giriş durumunu kontrol eder."""
    try:
        response = requests.get(f"http://127.0.0.1:{port}/", timeout=2)
        # Eğer login sayfasına yönlendirildiyse, kullanıcı giriş yapmamış
...
def run_gui(port):
    ...
    # Başlık
    title_label = tk.Label(main_frame, text="Yatırım Takip Uygulaması", 
    ...
    # Kullanıcı durumu etiketi
    user_status_label = tk.Label(main_frame, text="Kullanıcı durumu kontrol ediliyor...", 
                                font=("Arial", 10), fg="#95a5a6")
    user_status_label.pack(pady=5)

    def update_user_status():
        """Kullanıcı giriş durumunu günceller."""
        try:
            is_logged_in = check_login_status(port)
            if is_logged_in:
                user_status_label.config(text="✓ Kullanıcı giriş yapmış", fg="green")
            else:
                user_status_label.config(text="⚠ Giriş yapılması gerekiyor", fg="orange")
        except:
            user_status_label.config(text="⚡ Sunucu bağlantısı kontrol ediliyor...", fg="gray")
        
        # 5 saniye sonra tekrar kontrol et
        root.after(5000, update_user_status)
    ...
    def show_info():
        ...
        info_text = """Yatırım Takip Uygulaması - Güvenli Versiyon

Özellikler:
• Kullanıcı bazlı güvenli giriş sistemi
• Şifreli veri saklama
• Her kullanıcının kendi portföyü
• TEFAS, BIST, Altın ve Döviz takibi

Verileriniz güvenli bir şekilde aşağıdaki konumda saklanır:
{}

Güvenlik:
• Şifreler güvenli hash algoritmasıyla korunur
• Oturum yönetimi ile güvenli erişim
• Kullanıcı verileri izole edilmiş şekilde saklanır

İlk Giriş:
Demo hesap: admin / admin123
Yeni hesap oluşturabilirsiniz.

Geliştirici: Yatırım Takip Team
Sürüm: 2.0 (Güvenli)""".format(os.path.join(os.path.expanduser("~"), ".yatirim_takip"))
    ...
    # Kullanıcı durumu güncellemesini başlat
    root.after(2000, update_user_status)  # 2 saniye sonra başlat
...
if __name__ == "__main__":
    try:
        print("Yatırım Takip Uygulaması (Güvenli Versiyon) başlatılıyor...")
