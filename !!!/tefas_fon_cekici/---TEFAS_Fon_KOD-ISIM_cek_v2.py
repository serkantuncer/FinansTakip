...
class TefasFundClassifier:
    ...
    def export_specific_type_to_files(self, fund_type, base_name=None):
        """
        Belirli bir fon tipini hem JSON hem Excel olarak kaydet
        """
        if not base_name:
...
def get_specific_fund_type_data(fund_type):
    ...
    json_file, excel_file = classifier.export_specific_type_to_files(fund_type)
...
def main():
    ...
    print("\n=== DOSYALARA KAYDETME ===")
    json_file = classifier.export_to_json()
    excel_file = classifier.export_to_excel()
...
# Özelleştirilmiş kullanım örnekleri
def ozel_kullanim_ornekleri():
    ...
    # 1. Sadece Para Piyasası fonları
    print("\n1. SADECE PARA PİYASASI FONLARI:")
    print("-" * 40)
    json_file, excel_file = get_specific_fund_type_data('Para Piyasası')
...
def interaktif_kullanim():
    ...
    if selected_type:
        if selected_type == "Tümü":
            ...
            json_file, excel_file = get_specific_fund_type_data(selected_type)
...
def basit_kullanim():
    ...
    json_file, excel_file = classifier.export_specific_type_to_files('Para Piyasası')
